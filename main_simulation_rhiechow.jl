...
mesh = (
    NX = 32,                    # Number of x divisions for structured mesh 
    NY = 32                     # Number of y divisions for structured mesh 
)
...
solver = (
    ...
    SOR_TOL = 1e-11,             # SOR algorithm tolerance 
    SOR_BETA = 1.872            # SOR beta parameter 
)

x, y, X, Y, u, v, p, div = solve_cavity(domain, mesh, fluid_prop, bcs, solver)

writedlm("u_velocity_field_rhiechow.txt", u, ',')
writedlm("v_velocity_field_rhiechow.txt", v, ',')
writedlm("pressure_field_rhiechow.txt", p, ',')
writedlm("divergence_field_rhiechow.txt", div, ',')
writedlm("X_coordinate_field_rhiechow.txt", X, ',')
writedlm("y_coordinate_field_rhiechow.txt", Y, ',')
writedlm("x_coordinate_rhiechow.txt", x, ',')
writedlm("y_coordinate_rhiechow.txt", y, ',')