module cavity_functions

export initialize_variables, boundary_conditions!, stability_condition, intermediate_velocity!, intermediate_velocity_upwind! ,accel, calc_divergence, solve_pressure_poisson, solve_cavity
...
function intermediate_velocity_upwind!(u, v, u_old, v_old, rho, mu, nx, ny, dx, dy, dt)
    ...
    for i in 3:nx+1 # Iteration in x direction
        for j in 2:ny+1 # Iteration in y direction
            ...
            if ((v_old[j+1, i-1] + v_old[j+1,i]) > 0.0 ) #if (v_old[j,i] > 0.0)
                un = u_old[j,i]
                us = u_old[j-1,i]
                vn = 0.5 * (v_old[j+1, i-1] + v_old[j+1,i])
                vs = 0.5 * (v_old[j, i-1] + v_old[j,i])
